  ...
  return (
    <div className="relative w-full px-5 -mt-[70px] mb-20">
      ...
      >
        ...
        <h1 id="about" className="text-4xl font-semibold text-center pt-5 text-secondary bg-primary">
          zozo | create your own style
        </h1>
      </div>

      <div className="flex mt-28 gap-8 pl-14 flex-row-reverse">
        <div className="w-1/3 px-10 flex flex-col gap-4 pt-6">
          <h3 className="font-zozo text-5xl font-bold text-secondary">ZOZO</h3>
          <p className="text-2xl text-secondary ">
            {" "}
            لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ و با
            استفاده از طراحان گرافیک است. چاپگرها و متون بلکه روزنامه و مجله در
            ستون و سطرآنچنان که لازم است و تکنولوژی مورد نیاز و
            کاربردهای متنوع با هدف بهبود ابزارهای کاربردی می باشد.  حال و آینده شناخت فراوان جامعه و متخصصان را
            می طلبد ...{" "}
          </p>
        ...
        <div className="overflow-hidden flex flex-row-reverse gap-5 rounded-2xl bg-primary w-2/3  h-[450px] shadow-[13px_13px_24px_4px_#2b2b2b20]">
          <div className="w-1/2 h-full overflow-hidden">
            {" "}
            <img src={gif} className="w-full h-full object-cover" />
          </div>
          <p style={{direction:"ltr"}} className=" w-1/2 text-7xl font-semibold text-secondary-550 flex items-center px-10">LIFE ISN'T PERFECT BUT YOUR OUTFIT CAN BE.</p>
        </div>
      </div>
    </div>
  );
